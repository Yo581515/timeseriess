...
logger = get_logger("influxdb_cardinality_benchmark", readbench_config["general"]["log_file"])
...
for cfg in CARDINALITY_LEVELS:
    for run_num in range(1, NUMBER_OF_REPEATS + 1):
        observations, elapsed_ns = influxdb_repo.query_cardinality(
...
print("InfluxDB cardinality benchmark complete.")
influxdb_repo.close()