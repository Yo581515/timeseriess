...
# SETUP MONGODB CLIENT (readbench instance)
readbench_config = load_config("./configs/config-readbench-mongodb.yml")
logger = get_logger("mongodb_latest_value_benchmark", readbench_config["general"]["log_file"])
...
for parameter in PARAMETERS:
    for run_num in range(1, NUMBER_OF_REPEATS + 1):
        result_doc, elapsed_ns = mongodb_repo.query_latest(parameter)
        elapsed_seconds = elapsed_ns / 1_000_000_000
        result_found = result_doc is not None
...
print("MongoDB latest-value benchmark complete.")
