...
# Explicit, independent start/end pairs for each range window.
# Each window pulls from a different, non-overlapping slice of the
# 2011-08-13 -> 2026-08-13 dataset.
...
logger = get_logger("timescaledb_range_query_benchmark", readbench_config["general"]["log_file"])
...
for parameter in PARAMETERS:
    for window_name, window in RANGE_WINDOWS.items():
        ...
        for run_num in range(1, NUMBER_OF_REPEATS + 1):
            observations, elapsed_ns = timescaledb_repo.query_range(parameter, start_time, end_time)
...
print("TimescaleDB range-query benchmark complete.")
