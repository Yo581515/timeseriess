...
DISKTEST_CONTAINER_NAME = "timeseries-thesis-disk_timescaledb-1"
DISKTEST_DATA_PATH_IN_CONTAINER = "/var/lib/postgresql/data"
...
# Connect to the DISKTEST timescaledb instance
disktest_config = load_config("./configs/config-disktest-timescaledb.yml")
logger = get_logger("timescaledb_disk_size_benchmark", disktest_config["general"]["log_file"])
timescaledb_repo = TimescaleDBRepo(get_timescaledb_config(disktest_config["database"]), logger)

if timescaledb_repo.ping():
    print("Disktest TimescaleDB connection successful.")
print(lines)
...
try:
    with timescaledb_repo.cursor() as cur:  # adjust if your repo doesn't expose cursor() directly
        cur.execute("VACUUM;")
        cur.execute("CHECKPOINT;")
except Exception as e:
    logger.warning("Baseline vacuum/checkpoint skipped or failed: %s", e)
...
for batch_id, observations in files:
    ...
    # measure size BEFORE this batch's inserts
    ...
    while start < obs_len:
        ...
        timescaledb_repo.insert_many(chunk, page_size=BATCH_SIZE)
        start = end

    # force flush so the measurement reflects actual disk state, not buffered writes
    try:
        with timescaledb_repo.cursor() as cur:
            cur.execute("VACUUM;")
            cur.execute("CHECKPOINT;")
    except Exception as e:
        logger.warning("Vacuum/checkpoint failed for batch %d: %s", batch_id, e)

    # measure size AFTER this batch's inserts
...
# clear the observations table after the benchmark
try:
    timescaledb_repo.delete_all()
    print("Initial clear of observations table successful.")    
except Exception as e:
    logger.warning("Initial clear failed (table may not exist yet, that's fine): %s", e)

print("Disk size benchmark complete.")
